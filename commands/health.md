...
- **Consistency mechanics**: before judging by eye, run the deterministic checks, which cost no tokens and settle the mechanical half. `node ${CLAUDE_PLUGIN_ROOT}/assets/ci/example-continuity.mjs` for declared example values and placeholder spelling, `file-path-check.mjs` for a block a reader is told to write with no path in reach, and `duration-check.mjs` for a doc promising less time than its own steps require. Report what they find, and report a clean run as a clean run.
- **Coverage evidence**, when the project has a public issue tracker and the contributor opts in: run `doc-harvester` for what maintainers have already explained but never documented, and for questions asked more than twice. A recurring question with no doc behind it is a coverage gap with evidence attached, which beats any heuristic this scorecard can compute. Opt-in, and skip it silently when declined. See `${CLAUDE_PLUGIN_ROOT}/skills/docs-assist/reference/harvest.md`.
...
If the fix touches doc content and `lint.tools` is configured, re-run the linters against what changed before calling it done, and fix or note what they find. Don't let an edit made in this session become the next check's finding.
