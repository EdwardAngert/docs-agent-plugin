...
1. **No CI yet, but the repo has GitHub Actions**: recommend `/docs-assist:setup` to wire the link-check step into `.github/workflows/docs-lint.yml` (it scaffolds `markdown-link-check` with the right config) so this runs on every PR instead of once per audit. Still do a one-off pass this run (step 3) so the current audit isn't empty-handed.
...
- External link checking prefers, in order: an existing linter/CI setup already in the repo, wiring one up via `/docs-assist:setup` when CI exists, then an ad hoc `npx markdown-link-check` run. It needs `Bash` (for `npx`) or `WebFetch`; if neither is available, skip the check and say so rather than reporting links as clean
