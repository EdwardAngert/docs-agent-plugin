...
- When the target was a PR, offer to post the what-changed summary as a comment on it (`gh pr comment`), so the docs update is visible where the change is reviewed. Update any previous summary comment rather than stacking a new one.
