...
- Consolidate the fan-out: collect each drafter's report, add their proposed `llms.txt` entries yourself (subagents never edit shared files), apply the cross-references they suggest, then run `doc-auditor` once across the set as a silent second-opinion pass (fan it out further only if the set is large, per `audit-methodology.md`). It reads the drafts cold, not this conversation. Brief it with what each drafter already reported as SME-attested or verified against the code, so it spends its pass on what no single drafter could see rather than re-checking facts: terminology and example values drifting between siblings, since each drafted in isolation and only a set-wide pass catches inconsistency across them. Auto-apply what it flags as mechanical; fold anything needing judgment into the same list as the `needs-sme` flags rather than a separate report. Present the contributor one review queue.
