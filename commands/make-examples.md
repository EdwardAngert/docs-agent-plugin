...
Follow `${CLAUDE_PLUGIN_ROOT}/skills/docs-assist/reference/code-examples.md` for the method: examples that are safe and consistent with the rest of the docs, anchored to the `.docs-assist/reference.yml` registry.
...
- Read `.docs-assist/reference.yml` and use its `example-variable` values, or follow a `pointer` entry if one fits. If the registry is absent, offer to create it seeded from values already used across the docs, and add any new placeholder you introduce. The plugin maintains this file. See `code-examples.md`.
