...
- Check whether this doc mixes decay-heterogeneous content: a stable core section next to one making claims about a third-party system, a version-pinned value, or a subject a level removed from the rest of the doc. If so, offer section-level verification markers on the sections that stand apart, per `${CLAUDE_PLUGIN_ROOT}/skills/docs-assist/reference/section-verification.md` — a prompted option, not a default every doc gets. Most docs have no such mix and need only `last-verified`.
