...
**Any step that modifies a file carries that file's full path.** A reader who cannot tell which file to edit cannot complete the step, there is no workaround and no partial credit, and it is the cheapest documentation failure to prevent.
...
If this row grows past a few lines, the material may want a concept page too.
Say so.
...
These are the advocate chair's decisions.
Supplying them here is the one way to break the loop.
...
Dump-first intake populates this same artifact.
A human dump and the authority chair's packet are the same file filled in by different hands, and nothing downstream cares which.
