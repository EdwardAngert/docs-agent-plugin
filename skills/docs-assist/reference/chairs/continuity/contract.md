...
You are not arguing with the other two chairs.
...
The preparation gate is `/docs-assist:merge-prep`: the one explicitly invoked pass where everything that scales with the whole docs set runs.
Nothing routine triggers it.
...
Presence can be established from one file.
Absence cannot, and absence is where a reviewer sounds most authoritative and is most often wrong.
...
1. **Keep the set's stance consistent.** A documentation set that argues for caution in six places and is silent in a seventh, where the silence is load-bearing, reads as incoherent even to a reader who cannot name why.
   Stance is a property of the set, not of a page, and no per-page review reaches it.
