...
Specifically superseded: the the drafting and auditing subagents (both retired) subagents are retired and replaced by the authoring loop's chairs; the standalone second-opinion pass is subsumed by that loop; frontmatter is no longer load-bearing; and the four setup commands are merged into one.
...
- Shipped in 0.9.5: the the drafting subagent (retired) subagent drafts one planned doc from a plan entry plus persisted intake material, in parallel with its siblings, flagging what needs subject matter expert verification instead of inventing it.
...
- Added: `check-facts.mjs`, a deterministic checker (opt-in via `/docs-assist:setup`) for the mechanical half of `.docs-assist/reference.yml`: does a `fact` entry's source still contain the identifier it names, and does a `pointer` entry's target file and heading anchor still resolve. Comparing a fact's actual value against its source stays an agent-side check; that needs understanding the source language, which a deterministic script can't do generically.
...
- Added: `/docs-assist:setup` compiles every `term` entry into a generated Vale `substitution` rule (`styles/DocsAssist/Terminology.yml`), so terminology consistency also runs as a deterministic lint. The other three kinds stay agent-only: Vale doesn't check inside code blocks by default, ruling out `example-variable` and `fact`, and it can't follow a link to resolve a `pointer`.
